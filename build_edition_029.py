...
def build():
    ...
    stats = Table([[
        ...
        Paragraph("<b>WAR DAY 52</b>", styles["cover_value"]),
        Paragraph("<b>ESCALATING</b>", styles["cover_value"]),
    ]], colWidths=[35 * mm] * 5)
    ...
    counts = Table([[
        Paragraph(
            ...
            ParagraphStyle("cnt", fontName="Helvetica-Bold", fontSize=20, textColor=NAVY, alignment=TA_CENTER)
        ),
        Paragraph(
            "<b>3</b><br/><font size=7>ESCALATING</font>",
            ParagraphStyle("cnt2", fontName="Helvetica-Bold", fontSize=20, textColor=RED_TAG, alignment=TA_CENTER)
        ),
        Paragraph(
            "<b>2</b><br/><font size=7>CONFIRMED THIS CYCLE</font>",
            ParagraphStyle("cnt3", fontName="Helvetica-Bold", fontSize=20, textColor=AMBER, alignment=TA_CENTER)
        ),
        Paragraph(
            "<b>4</b><br/><font size=7>LIVE WINDOWS</font>",
            ParagraphStyle("cnt4", fontName="Helvetica-Bold", fontSize=20, textColor=TEAL, alignment=TA_CENTER)
        ),
    ]], colWidths=[43.5 * mm] * 4)
    ...
    story.append(Paragraph(
        "<b>PRED-026-C / PRED-028-C: CONFIRMED.</b> "
    ...
    story.append(Paragraph(
        "<b>PRED-022-C / PRED-023-B / PRED-026-D: CONFIRMED (carry-forward).</b> "
    ...
    story.append(Paragraph(
        "<b>FACT:</b> Momentum toward a second round is real, but the formal threshold remains unmet. "
        "This creates a calibration problem: rhetorical momentum is high, evidentiary closure is not.",
        styles["body"]
    ))
    story.append(Paragraph(
        "<b>INFERENCE:</b> H5 remains primary because the structural contradiction between red lines is unresolved. "
        "H6 remains active because the same unresolved diplomatic state interacts with Hormuz, GL U, and Yanbu risk.",
        styles["body"]
    ))
    story.append(Paragraph(
        "<b>AFC:</b> The strongest steelman for H-A1 is that the parties still appear to want a second round before the ceasefire forcing function expires.",
        styles["afc_note"]
    ))
    story.append(Paragraph(
    ...
    story.append(Paragraph(
        "<b>1. Key assumption:</b> The diplomatic momentum reflected in official signalling still corresponds to a real path toward a second round rather than narrative management alone.",
        styles["body"]
    ))
    story.append(Paragraph(
        "<b>2. Fragile fact:</b> The absence of a formal second-round announcement is decisive; if a named date, venue, and agenda were published, H-A1 would move materially higher.",
        styles["body"]
    ))
    story.append(Paragraph(
        "<b>3. High-impact uncertainty:</b> Whether a formal second-round announcement appears before the ceasefire deadline.",
        styles["body"]
    ))
    story.append(Paragraph(
        "<b>4. Tier 4 dependency test:</b> YES — the dominant hypothesis still holds without Tier 4 support because the probability is sustained by carry-forward Tier 1/2 reporting and formal-threshold logic.",
        styles["body"]
    ))
    ...
    ], "Gate 5 trigger: formal diplomatic collapse or ceasefire lapse without extension requires full Case A reframe.", styles))
    ...
    story.append(Paragraph("PART 2 — OBSERVED INCONGRUITY", styles["section_head"]))
    story.append(Paragraph(
        "<b>FACT:</b> The framework held through the first major monitoring window, but only with sub-escalatory violations still present.",
        styles["body"]
    ))
    story.append(Paragraph(
        "<b>INFERENCE:</b> H-C1 now has the stronger position because operational pause is not total peace, but enough suppression exists to keep collapse risk lower.",
        styles["body"]
    ))
    story.append(Paragraph(
        "<b>AFC:</b> The strongest case against H-C1 is that ceasefire text and political pressure can still unravel quickly if either side escalates beyond current violation levels.",
        styles["afc_note"]
    ))
    story.append(Paragraph(
    ...
    story.append(Paragraph("PART 4 — TRUTH QUALITY CHECK", styles["section_head"]))
    story.append(Paragraph(
        "<b>1. Key assumption:</b> Sub-escalatory violations remain bounded and do not convert into a major barrage or large-scale strike.",
        styles["body"]
    ))
    story.append(Paragraph(
        "<b>2. Fragile fact:</b> The classification depends on the absence of a qualifying major violation during the 72-hour window.",
        styles["body"]
    ))
    story.append(Paragraph(
        "<b>3. High-impact uncertainty:</b> Whether Trump’s prohibition signal remains politically binding if Netanyahu pressure intensifies.",
        styles["body"]
    ))
    story.append(Paragraph(
        "<b>4. Tier 4 dependency test:</b> YES — the dominant hypothesis survives without Tier 4 because the key logic is the absence of major escalation and the carried ceasefire state.",
        styles["body"]
    ))
    ...
    ], "Gate 5 trigger: major barrage or large-scale strike requires full Case C reframe.", styles))
